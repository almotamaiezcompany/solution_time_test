    ...
    <header class="c-header c-header-light c-header-fixed c-header-with-subheader">
        ...
        <div class="dropdown-menu dropdown-menu-right pt-0">
            @if(\Illuminate\Support\Facades\Auth::user())
                ...
                <ul class="nav">
                    @foreach(\Illuminate\Support\Facades\Auth::user()->notifications()->limit(10)->get() as $notification)
                        <li class="nav-item w-100 p-2"
                            style="background-color: {{$notification->read_at ? '#fff':'#e6e6e6'}}"> {{$notification->data['title']}}</li>
                    @endforeach
                </ul>
            @endif
        </div>
